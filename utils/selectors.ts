...
export async function openBoard(page: Page, boardName: string) {
    ...
    await expect(board, `Board "${boardName}" not visible`).toBeVisible({ timeout: 10_000 });
    await board.click();
...
export function taskCardIn(region: Locator, taskTitle: string): Locator {
    const title = exactTextRegex(taskTitle);
    console.log(`Looking for task title: ${taskTitle}`);

    // Cards container within the column region
...
export async function expectTagsIn(card: Locator, tags: string[]) {
    for (const tag of tags) {
        ...
        await expect(tagLoc, `Tag "${tag}" not visible on card`).toBeVisible({ timeout: 8_000 });
    }
}
...
export async function expectTaskWithTags(
    ...
    await expect(region, `Column "${columnName}" not found`).toBeVisible({ timeout: 10_000 });

    await page.waitForSelector('div.bg-white.p-4.rounded-lg.shadow-sm.border.border-gray-200', { state: 'visible' });
    
    const card = taskCardIn(region, taskTitle);
    

    await expect(card.getByRole('heading', { name: exactTextRegex(taskTitle) })).toBeVisible();
